...
import cn.shang.promotion.pojo.TimeRangeMergeResult;
...
public class FreeTimeRangeMerger {
    ...
    public TimeRangeMergeResult merge(List<FreeTimeRange> timeRanges,
        ...
        // 1. 预处理：截取在整体区间内的部分
        List<FreeTimeRange> processedSlots = preprocessSlots(timeRanges, overallStart, overallEnd, result);
        if (processedSlots.isEmpty()) {
            return result;
        }

        // 2. 按优先级排序（优先级高的在前）
        processedSlots.sort(Comparator.comparingInt(FreeTimeRange::getPriority)
                .thenComparing(FreeTimeRange::getBeginTime));

        // 3. 按优先级分组处理
        Map<Integer, List<FreeTimeRange>> timeRangesByPriority = new TreeMap<>();
        for (FreeTimeRange slot : processedSlots) {
            timeRangesByPriority.computeIfAbsent(slot.getPriority(), k -> new ArrayList<>()).add(slot);
        }
     ...
     */
    private List<FreeTimeRange> preprocessSlots(List<FreeTimeRange> timeRanges,
                                                LocalDateTime overallStart,
                                                LocalDateTime overallEnd,
    ...
    private List<FreeTimeRange> finalizeResult(List<FreeTimeRange> timeRanges) {
        ...
        return result.stream()
                .filter(tr -> tr.getEndTime().isAfter(tr.getBeginTime()))
