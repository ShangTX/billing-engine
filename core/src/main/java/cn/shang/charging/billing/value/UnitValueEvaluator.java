...
public final class UnitValueEvaluator {
    ...
    public static UnitValueProjection evaluate(
        ...
        if (projection.currentAmount().signum() < 0) {
            throw new IllegalStateException("currentAmount must be >= 0");
        }
        if (projection.nextChangeTime().isBefore(queryTime)) {
            throw new IllegalStateException("nextChangeTime must be >= queryTime");
        }
        if (projection.nextChangeTime().isAfter(unitEndTime)) {
            throw new IllegalStateException("nextChangeTime must be <= unitEndTime");
        }
        if (queryTime.isBefore(unitEndTime) && projection.nextChangeTime().equals(queryTime)) {
            throw new IllegalStateException("nextChangeTime must advance when queryTime is before unitEndTime");
        }
        return projection;
    }
}
