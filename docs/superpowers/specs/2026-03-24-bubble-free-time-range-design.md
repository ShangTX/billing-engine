...
├── 2. 过滤已使用免费时段（复用 filterUsedFreeRanges），得到可用免费时段
├── 3. 计算并生成计费单元，同时确定本次使用的气泡部分
├── 4. 计算周期边界延长 = 本次使用的气泡型时段总长度
├── 5. cycleBoundary = 原始周期边界 + 周期延长
├── 6. 更新 usedFreeRanges（包含本次使用的气泡部分）
└── 7. 输出 cycleBoundary（已延长）
```

**关键说明**：
- 周期延长基于**本次使用的气泡型时段**，而非恢复的 usedFreeRanges
- usedFreeRanges 的作用是过滤已用部分，避免重复使用
- 气泡时长累积效果通过 cycleBoundary 结转实现
...
```
新 cycleBoundary = 当前 cycleBoundary + 本次使用气泡时长
```

- 当前 cycleBoundary：从 RuleState 恢复，已包含之前的延长
- 本次使用气泡时长：当前计算窗口内实际使用的气泡型时段长度

**关键点**：
- 气泡部分使用：只对计算窗口内的部分延长周期
- 结转未用部分：通过 `usedFreeRanges` 过滤自动实现
- 累积延长：cycleBoundary 每次累加本次气泡延长
